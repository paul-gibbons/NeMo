...
class TarOrFolderVideoLoader:
    ...
    def flatten_frames(self, cap):
        if self.data_cfg['splice_single_frame'] == 'first':
            ...
            logging.debug(f"Extracting first frame from video")
            return Image.fromarray(frame).convert('RGB')
        elif self.data_cfg['splice_single_frame'] == 'middle':
            frame = cap[len(cap) // 2].asnumpy()[:, :, ::-1]
            logging.debug(f"Extracting middle frame from video")
            return Image.fromarray(frame).convert('RGB')
        elif self.data_cfg['splice_single_frame'] == 'last':
            frame = cap[-1].asnumpy()[:, :, ::-1]
            logging.debug(f"Extracting last frame from video")
            return Image.fromarray(frame).convert('RGB')
        else:
            if self.data_cfg['num_frames'] == -1:
                ...
                for frame in cap:
                    ...
                    frames.append(img)
                logging.debug(f"Extracting all {len(frames)} frames from video")
                return frames
            else:
                ...
                for i in range(0, len(cap), partition_size):
                    ...
                    img = Image.fromarray(rgb_frame).convert('RGB')
                    frames.append(img)
                logging.debug(f"Extracting {len(frames)} frames from video based on num_frames={self.data_cfg['num_frames']}")
                return frames
...
def tokenize(
    ...
    logging.debug(f"Adjusted context length: {context_length}")
    ...
    for i, token in enumerate(tokens):
        if len(token) > context_length + add_extra_token:
            ...
            logging.debug(f"Truncated token length for text {i}: {len(token)}")
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            for video_file in self.list_data_dict[i]['video']:
                ...
                logging.debug(f"Loaded video {video_file} with {len(frames)} frames")
                if isinstance(self.processor, CLIPImageProcessor):
                    # image processor from HF
                    if self.multimodal_cfg['image_aspect_ratio'] == 'keep':
            ...
            if videos:
                ...
                logging.debug(f"Video tensor shape: {media_tensors.shape}")
                logging.debug(f"Current token length: {cur_token_len}")
                sources = preprocess_multimodal(
                    copy.deepcopy(sources),
                    self.multimodal_cfg,
                    cur_token_len,
